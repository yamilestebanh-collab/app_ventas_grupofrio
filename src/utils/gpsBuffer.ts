...
//
// V2: reduced from 500 to 200. With batch sync (50/batch), 200 points
// = 4 batches. Even concurrent-5 fallback clears 200 in ~40 tandas.
// 200 still covers a full shift at 1 point/min rate limit.
export const GPS_QUEUE_CAP = 200;

// V2: accuracy gate — reject GPS points with accuracy worse than this.
// Check-in/check-out are NOT blocked by this (they are independent of
// the GPS buffer). This only affects telemetry GPS points.
export const GPS_MIN_ACCURACY_M = 50;
...
export type GpsAdmissionResult =
  ...
  | { accept: false; reason: 'low-accuracy' };
...
export function shouldAdmitGpsPoint(
  ...
  try {
    ...
    // V2: accuracy gate — reject points with accuracy worse than threshold.
    // Does NOT apply to check-in/check-out GPS (those bypass the buffer).
    if (
      typeof candidate.accuracy === 'number' &&
      candidate.accuracy > GPS_MIN_ACCURACY_M
    ) {
      gpsBufferCounters.invalidTotal += 1;
